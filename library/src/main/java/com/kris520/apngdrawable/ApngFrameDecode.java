...
public class ApngFrameDecode {
    ...
    public void prepare() {
        ...
        //if (enableDebugLog) Log.d(TAG, "prepare imagePath:" + imagePath);
        if (imagePath == null) return;

        baseFile = new File(imagePath);
        if (!baseFile.exists()) return;

        //if (enableDebugLog) Log.d(TAG, "Extracting PNGs..");
        ApngExtractFrames.process(baseFile);
        //if (enableDebugLog) Log.d(TAG, "Extracting complete");

        //if (enableDebugLog) Log.d(TAG, "Read APNG information..");
    ...
    Bitmap createFrameBitmap(final int frameIndex) {
        ...
        //if (enableDebugLog) Log.d(TAG, "EXIT");
    ...
    private void readApngInformation(File apngFile) {
        ...
        for (int i = 0; i < pngChunks.size(); i++) {
            ...
            if (chunk instanceof PngChunkACTL) {
                ...
                if (ApngDrawable.enableDebugLog) Log.d(ApngDrawable.TAG, "frameCount: " + frameCount + ", playCount:" + playCount);
        ...
        if (ApngDrawable.enableDebugLog) Log.d(ApngDrawable.TAG, "maxCacheSize: " + maxCacheSize);
